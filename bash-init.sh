...
_terminal_hud_update_status() {
    ...
    tmux set-option -w -t "$TMUX_PANE" @tab_age " $(_terminal_hud_age) " 2>/dev/null
}
...
_terminal_hud_arm() {
    _TERMINAL_HUD_NEXT_CMD=1
}

_terminal_hud_debug() {
    # _TERMINAL_HUD_NEXT_CMD is set by _terminal_hud_arm (last in PROMPT_COMMAND);
    # the very first DEBUG fire after the prompt is the user's command.
    [ "$_TERMINAL_HUD_NEXT_CMD" != "1" ] && return
    _TERMINAL_HUD_NEXT_CMD=0
    [ -z "$TMUX" ] && return
    local dir="${PWD##*/}"
...
trap '_terminal_hud_debug' DEBUG
# Prepend status update; append the arm so VTE/other hooks don't get caught.
PROMPT_COMMAND="_terminal_hud_update_status${PROMPT_COMMAND:+; $PROMPT_COMMAND}; _terminal_hud_arm"
...
ssh() {
    ...
    remote_host=$(_terminal_hud_ssh_hostname "$@")
...
# so the terminal tab stays open.  Killing the grouped session
# disconnects the client, which causes the terminal to close the tab.
_terminal_hud_exit() {
    # Read the grouped session name that terminal-shell stamped on this window.
    # We can't use #{session_name} because $TMUX in the first tab's shell points
    # to the base "terminal-hud" session, which must never be killed.
    local session
    session=$(tmux show-options -wqv -t "$TMUX_PANE" @grouped_session 2>/dev/null)
    if [ -n "$session" ]; then
        # Killing the grouped session disconnects the tmux client → the
        # terminal closes the tab.  The window is cleaned up automatically
        # by tmux once this shell process exits.
        tmux kill-session -t "$session" 2>/dev/null
    fi
}
trap '_terminal_hud_exit' EXIT
